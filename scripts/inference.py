...
def main():
    ...
    parser.add_argument('--clip_model', type=str, default='openai/clip-vit-base-patch16', 
                        help='OpenAI CLIP model name or path to local model file')
    ...
    # Check if clip_model is a local file path
    if os.path.isfile(args.clip_model):
        print(f"Using local CLIP model: {args.clip_model}")
    else:
        print(f"Using CLIP model name: {args.clip_model}")
    ...
    classifier = CLIPAdapter(args.clip_model, device=args.device)
