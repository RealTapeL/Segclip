...
class FastSAMPrompt:
    ...
    def __init__(self, image, results, device='cuda'):
        ...
        self.img = image
        self.img_array = image  # 添加这行以修复错误
    ...
    def plot_to_result(
        ...
        original_h = self.img_array.shape[0]
        original_w = self.img_array.shape[1]
        ...
        if retina:
            annotations = self._scale_annotations(
                ...
                target_h=self.img_array.shape[0],
                target_w=self.img_array.shape[1],
            )
        if bboxes is not None:
            bboxes = self._scale_annotations(
                bboxes,
                original_h=original_h,
                original_w=original_w,
                target_h=self.img_array.shape[0],
        ...
        if mask_random_color:
            colors = np.random.rand(len(annotations), 3)
        else:
            colors = np.ones((len(annotations), 3)) * np.array([30/255, 144/255, 255/255])
        ...
        for i, mask in enumerate(annotations):
            ...
            # 创建彩色掩码
            color = colors[i] if mask_random_color else colors[i]
            colored_mask = np.zeros((original_h, original_w, 4))
            colored_mask[annotation == 1] = [color[0], color[1], color[2], 0.6]  # RGBA
        ...
        try:
            ...
            try:
                ...
                # If both methods fail, draw and try again
                fig.canvas.draw()
                buf = fig.canvas.buffer_rgba()
                cols, rows = fig.canvas.get_width_height()
                img_array = np.frombuffer(buf, dtype=np.uint8).reshape(rows, cols, 4)
