...
def torch_safe_load(weight):
    ...
    try:
        ...
        if e.name == 'models':
            raise TypeError(
                emojis(f'ERROR ❌️ {weight} appears to be an Ultralytics YOLOv5 model originally trained '
                       f'with <url id="cvl0n0dbf30f86dgiv0g" type="url" status="parsed" title="GitHub - ultralytics/yolov5: YOLOv5 🚀 in PyTorch &gt; ONNX &gt; CoreML &gt; TFLite" wc="16728">https://github.com/ultralytics/yolov5</url> .\nThis model is NOT forwards compatible with '
                       f'YOLOv8 at <url id="cvl0n0dbf30f86dgiv10" type="url" status="parsed" title="GitHub - ultralytics/ultralytics: Ultralytics YOLO11 🚀" wc="11179">https://github.com/ultralytics/ultralytics</url> .'
                       f"\nRecommend fixes are to train a new model using the latest 'ultralytics' package or to "
                       f"run a command with an official YOLOv8 model, i.e. 'yolo predict model=yolov8n.pt'")) from e
        LOGGER.warning(f"WARNING ⚠️ {weight} appears to require '{e.name}', which is not in ultralytics requirements."
                       f"\nAutoInstall will run now for '{e.name}' but this feature will be removed in the future."
